...
time_func() {
    ...
    echo "Временной диапазон анализа скрипта $timestamp_last - $date_curr" >> mail.txt
...
parsing() {
    echo ""
    #Подсчет ip-адрессов
    echo "Статистика запросов от ip-адрессов:" >> mail.txt
    tail -n+$num_old $logfile | awk -F"-" '{print $1}' | sort -nr | uniq -c | sort -nr >> mail.txt
    echo "_____________________________________________" >> mail.txt
    #Подсчет запрашиваемых страниц
    echo "Статистика запрашиваемых адрессов:" >> mail.txt
    tail -n+$num_old $logfile | awk -F'"' '{print $2}' | awk -F " " '{print $2}' | sort -nr | uniq -c | sort -nr >> mail.txt
    echo "_____________________________________________" >> mail.txt
    #Подсчет кодов возврата
    echo "Статистика кодов возврата:" >> mail.txt
    tail -n+$num_old $logfile | awk -F'"' '{print $3}' | awk -F " " '{print $1}' | sort -nr | uniq -c | sort -nr >> mail.txt
    echo "_____________________________________________" >> mail.txt
    #Подсчет кодов ошибок
    echo "Статистика кодов ошибок:" >> mail.txt
    tail -n+$num_old $logfile | awk -F'"' '{print $3}' | awk -F " " '{print $1}'| awk '/[4-5][0-9][0-9]/' | sort -nr | uniq -c | sort -nr >> mail.txt
    echo "_____________________________________________" >> mail.txt
    echo "Прочитано $n строк из $logfile" >> mail.txt
}
